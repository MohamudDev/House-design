...
import { Home, Users, ShieldCheck, ChevronRight, Layout, DollarSign, Eye } from 'lucide-react';
...
const Landing = () => {
  ...
  useEffect(() => {
    const fetchPublicDesigns = async () => {
      try {
        ...
        setDesigns(data.data.slice(0, 6)); // Show top 6
      } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-white">
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full px-8 flex flex-col items-start">
          <div className="max-w-2xl text-left animate-in fade-in slide-in-from-left duration-1000">
            ...
            <h1 className="text-6xl md:text-8xl font-black text-white leading-[1.1] mb-8 tracking-tighter">
              Your Vision, <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400">Our Blueprint.</span>
            </h1>
            
            <p className="text-xl text-slate-300 mb-12 leading-relaxed max-w-xl font-medium">
              Browse world-class 3D house designs. Connect with verified engineers and start your building journey today.
            </p>
            
            <div className="flex flex-col sm:flex-row gap-5">
              <Link 
                to={user?.role === 'client' ? '/client-dashboard' : '/register'} 
              ...
              <a 
                href="#gallery"
                className="bg-white/10 hover:bg-white/20 backdrop-blur border border-white/20 text-white px-10 py-5 rounded-2xl font-black text-lg flex items-center justify-center transition-all active:scale-95"
              >
                View Gallery
              </a>
            </div>
      ...
      <div className="bg-slate-50 py-12 border-y border-slate-200">
        <div className="max-w-7xl mx-auto px-8 grid grid-cols-1 md:grid-cols-3 gap-12 text-center">
          <div className="flex flex-col items-center">
            ...
            <h3 className="text-slate-900 font-bold text-lg">Verified Designs</h3>
            <p className="text-slate-500 text-sm mt-2">Every plan is checked for quality</p>
          </div>
          <div className="flex flex-col items-center">
            <div className="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-4 border border-slate-100">
              <Users size={32} className="text-indigo-600" />
            </div>
            <h3 className="text-slate-900 font-bold text-lg">Direct Chat</h3>
            <p className="text-slate-500 text-sm mt-2">Talk directly with the engineer</p>
          </div>
          <div className="flex flex-col items-center">
            <div className="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-4 border border-slate-100">
              <ShieldCheck size={32} className="text-indigo-600" />
            </div>
            <h3 className="text-slate-900 font-bold text-lg">Secure Process</h3>
            <p className="text-slate-500 text-sm mt-2">Safe transactions and vetted pros</p>
          </div>
        </div>
      </div>

      {/* Featured Gallery */}
      <section id="gallery" className="py-24 px-8 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-4 tracking-tight">Featured Collections</h2>
            <div className="w-24 h-2 bg-indigo-600 mx-auto rounded-full mb-6"></div>
            <p className="text-slate-500 text-lg max-w-2xl mx-auto">Discover the most popular architectural trends handpicked by our experts.</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-slate-50 rounded-[2.5rem] p-20 text-center border-2 border-dashed border-slate-200">
              <p className="text-slate-400 font-bold text-xl uppercase tracking-widest">Gallery Coming Soon</p>
              <p className="text-slate-500 mt-2 italic">Engineers are currently preparing stunning 3D models for you.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              {designs.map((design) => (
                    ...
                    <div className="absolute inset-0 bg-slate-100">
                      {design.images && design.images.length > 0 ? (
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center text-slate-300">
                          <div className="w-20 h-20 bg-white rounded-3xl flex items-center justify-center shadow-inner mb-4">
                            <Layout size={40} />
                          </div>
      ...
      <footer className="bg-slate-900 py-20 px-8 text-white border-t border-white/10">
        ...
        <div className="max-w-7xl mx-auto mt-20 pt-10 border-t border-white/5 text-center text-slate-500 font-bold text-sm">
